...
public class LocationsSqlHelper extends SQLHelper {
    ...
    public ArrayList<Coordinates> getLocationsByShift(Shift shift) {
        ...
        String selectQuery = "SELECT  * FROM " + TABLE_NAME + " WHERE "+DATE_TIME+"<='"+dateFormat.format(toTime)+"' AND "+DATE_TIME+">='"+dateFormat.format(fromTime)+"'";
        ...
        if (cursor.moveToFirst()) {
            do {
                coordinates.add(loadLocationFromCursor(cursor));
