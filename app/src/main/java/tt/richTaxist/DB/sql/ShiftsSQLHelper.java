...
public class ShiftsSQLHelper extends SQLHelper {
    ...
    public boolean commit(Shift shift){
        ...
        if (shift.endShift!=null) cv.put(END_SHIFT, dateFormat.format(shift.endShift)); else cv.put(END_SHIFT, "");
    ...
    public ArrayList<Shift> getShifts(Date fromDate, Date toDate, boolean youngIsOnTop) {
        ...
        // Select All Query
        String selectQuery = "SELECT  * FROM " + TABLE_NAME + " WHERE "+BEGIN_SHIFT+"<='"+dateFormat.format(toDate)+"' AND "+BEGIN_SHIFT+">='"+dateFormat.format(fromDate)+"' ORDER BY "+BEGIN_SHIFT+" "+sortMethod;
        ...
        if (cursor.moveToFirst()) {
            do {
                shiftsStorage.add(loadShiftFromCursor(cursor));
