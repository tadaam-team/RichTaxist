...
public class OverlayRect extends Overlay {
    ...
    private static final String LOG_TAG = "overlay rect";
    OverlayRectItem overlayRectItem;
    Context mContext;
    MapController mMapController;
    RectRender rectRender; 
     
    public OverlayRect(MapController mapController, List<GeoPoint> coords) {
        super(mapController);
        ...
        for (int i = 0; i < coords.size(); i++) {
            GeoPoint geoPoint = coords.get(i);
            overlayRectItem.geoPoint.add(geoPoint);
        }
        Log.d(LOG_TAG,"points count: "+String.valueOf(coords.size()));
